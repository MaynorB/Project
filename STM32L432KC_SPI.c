...
void SD_Select(void) {
    digitalWrite(SPI_CE, 0); // Assert chip select (active low)
}

void SD_Deselect(void) {
    digitalWrite(SPI_CE, 1); // Deassert chip select
    spiSendReceive(0xFF);    // Give one extra clock
}

uint8_t SD_SPI_Transmit(uint8_t data) {
    return (uint8_t) spiSendReceive((char)data);
}

uint8_t SD_SPI_Receive(void) {
    return (uint8_t) spiSendReceive(0xFF); // Dummy byte to read
}
